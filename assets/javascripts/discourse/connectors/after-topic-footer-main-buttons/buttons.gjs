...
import { debug, error } from "@ember/debug";
...
export default class CloseNotifyComponent extends Component {
    ...
    {{#if this.shouldRender}}
        {{#unless this.closed}}
            <DButton
                @icon={{this.deployIcon}}
                @label={{this.deployLabel}}
                @title={{this.deployTitle}}
                @class='btn-icon {{this.deployClass}}'
                {{on 'click' this.doDeploy}}
            />
        {{/unless}}
        <DButton
            @icon={{this.closeIcon}}
